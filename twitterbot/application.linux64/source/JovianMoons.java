...
public class JovianMoons extends PApplet {
...
public void calculateMoons(int yr, int mon, int da, int hr, int min, int sec){
  float J2000 = J2000DaysFromUTCTime(yr, mon, da, hr, min, sec);
  //float J2000 = -2572.500000;
...
public void setup(){
  ...
  hour += 1;  // server is in Virginia, move it to UTC
  // expecting a +5, but testing against my computer and +1 is correct
  correctDates();
  ...
  fill(255);
  noStroke();
  image(jupiterImage, centerX - SCALE*2.0f*0.5f, centerY - SCALE*2.0f*0.5f, SCALE*2.0f, SCALE*2.0f);
  //ellipse(centerX, centerY, SCALE*2.0, SCALE*2.0);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "JovianMoons" };
    if (passedArgs != null) {
